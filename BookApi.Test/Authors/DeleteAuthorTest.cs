...
public class DeleteAuthorTest : AuthorUseCaseTestBase
{
    ...
    public async Task 正常系_著者に関連付けられた書籍を削除()
    {
        ...
        // Assert
        // 著者とそれに関連付けられた書籍が削除
        DbContext.AssertNotExistData<AuthorDataModel>(bocchi.Id);
        DbContext.AssertNotExistData<BookDataModel>(book1.Id);

        // 中間テーブルも削除
        DbContext.AssertNotExistData<BookAuthorDataModel>(x => x.BookId == book1.Id);

        // 出版社、削除された本の他の著者、関係のない書籍には影響しない
        DbContext.AssertData(nijika.Id, nijika);
        DbContext.AssertData(ryou.Id, ryou);
        DbContext.AssertData(ikuyo.Id, ikuyo);
        DbContext.AssertData(publisher.Id, publisher);
        DbContext.AssertData(book2.Id, book2);
        DbContext.Set<BookAuthorDataModel>()
            .Where(x => x.BookId == book2.Id)
            .Select(x => x.AuthorId)
            .Should()
    ...
    public async Task 正常系_他の管理者が作成したデータを削除できる()
    {
        ...
        var book = DbContext.AddBookToDatabase(
            UserFixture.Admin, CreatedAt, "000-0-00-000000-0", "A book to delete", [1, 2], 1
        );
        ...
        // 著者とそれに関連付けられた書籍が削除
        DbContext.AssertNotExistData<AuthorDataModel>(bocchi.Id);
        DbContext.AssertNotExistData<BookDataModel>(book.Id);
